...
from res_inner_nabp import res_INABP, BasicBlock, BasicBlockINABP, Bottleneck


class ResINABPWrapper(res_INABP):
    ...
    def __init__(self,
        ...
        if n_layer == 34:
            super(ResINABPWrapper,
                  self).__init__(BasicBlock, BasicBlockINABP, [3, 4, 6, 3],
                                 n_class, dropout, short_len)
            self.load_pretrained_weights('resnet34')
        elif n_layer == 50:
            super(ResINABPWrapper,
                  self).__init__(Bottleneck, BasicBlockINABP, [3, 4, 6, 3],
                                 n_class, dropout, short_len)
    ...
    def forward(self, x):
        ...
        x = self.maxpool2(x)
        x = x.transpose(1, 2).contiguous()
        # print('after before transpose maxpool2.size:', x.size())
        x = x.view(b * (short_len // 2 - 1), dim2, h2, w2)
